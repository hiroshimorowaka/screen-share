...
macro_rules! string_newtype {
    ($(#[$meta:meta])* $name:ident) => {
        ...
        // Deref to `str` like `String` does, so `&PeerId` coerces to
        // `&str` at call sites that still take `&str`. The inner value is
        // validated at construction; exposing the `str` API on top of it
        // is read-only and cannot bypass that.
        ...
        // Ergonomic string comparisons — mainly for tests that assert a
        // wire value against a literal. `==` against a raw string never
        // sits on a trust boundary (that path always goes through
        // `parse`), so this only saves ceremony, it does not weaken a
        // check.
