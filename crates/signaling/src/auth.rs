...
/// could OOM the process and drop every in-memory room (finding F02). This
/// is OWASP's lowest-memory Argon2id profile — 7 MiB / t=5 / p=1 — which
/// keeps brute-force resistance equivalent to the 19 MiB / t=2 default
/// while cutting the per-call footprint by ~2.7x.
...
///
/// Verification still honours whatever parameters are embedded in the
/// stored PHC string, so hashes written with the old `Argon2::default()`
/// cost keep verifying after this change.
