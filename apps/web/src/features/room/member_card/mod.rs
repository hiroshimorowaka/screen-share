...
use super::media_controls::{
    exit_fullscreen_if_active, set_muted, toggle_fullscreen, toggle_picture_in_picture, VideoSlot,
};
...
mod parts;
use parts::{QualityMenu, VolumeControl};
...
/// affordances, and the per-stream action bar.
//
// Over the line lint: a card is a video tile plus a badge cluster, a watch
// scrim, a footer and an action bar, each with its own reactive
// visibility — the length is markup, not branching. The two stateful
...
fn MemberCard(conn: RoomSession, signals: MemberCardSignals, index: usize) -> impl IntoView {
    ...
    let set_quality =
        crate::session::quality::set_quality_handler(conn.clone(), members, quality_by_peer, i);
            ...
            <div class="card__footer">
                ...
                <div class="card__actions">
                    ...
                    <QualityMenu
                        ...
                        is_touch=is_touch
                        current=Signal::derive(current_quality)
                        on_select=set_quality
                    />
                    <VolumeControl
                        hidden=Signal::derive(move || !is_watching_this())
                        is_muted=Signal::derive(is_muted)
                        volume_pct=Signal::derive(current_volume_pct)
