...
fn drop_focus_if_showing(expanded: RwSignal<Option<String>>, peer_id: &str) {
    let was_fullscreen = crate::room::media_controls::exit_fullscreen_if_showing(peer_id);
...
/// A member other than us joined mid-session — append them to the roster
/// and let the desktop shell raise a notification.
#[cfg(feature = "hydrate")]
fn on_peer_joined(signals: RoomState, peer_id: String, nick: String, color: String) {
    crate::client::desktop_bridge::notify_desktop_member_joined(&nick);
    signals.set_members.update(|members| {
...
/// A member left — drop them from the roster and tear down any connection
/// either direction, plus any focus that was on their card.
#[cfg(feature = "hydrate")]
fn on_peer_left(conn: &RoomSession, signals: RoomState, peer_id: String) {
    signals
        .set_members
...
#[cfg(feature = "hydrate")]
fn set_peer_sharing(signals: RoomState, peer_id: &str, sharing: bool) {
    signals.set_members.update(|members| {
...
/// A sharer stopped — clear the sharing flag, drop us as a watcher, tear
/// down our incoming connection to them, and unfocus their card.
#[cfg(feature = "hydrate")]
fn on_peer_stopped_sharing(conn: &RoomSession, signals: RoomState, peer_id: String) {
    set_peer_sharing(signals, &peer_id, false);
    signals.watching.update(|w| {
        w.remove(&peer_id);
    });
    drop_focus_if_showing(signals.expanded, &peer_id);
...
/// `take()`, not `get()`: an unmatched `Pong` (one arriving after a fresh
/// `Ping` overwrote the timestamp) must not be timed against the wrong send.
#[cfg(feature = "hydrate")]
fn on_pong(conn: &RoomSession) {
...
/// arm either updates a room signal directly or hands off to a `fn`
/// above; the negotiation-heavy arms live entirely in those.
#[cfg(feature = "hydrate")]
pub(crate) fn build_message_handler(
    ...
    move |msg: ServerMessage| match msg {
        ...
        ServerMessage::PeerLeft { peer_id } => on_peer_left(&conn, signals, peer_id.to_string()),
        ServerMessage::PeerStartedSharing { peer_id } => {
            set_peer_sharing(signals, peer_id.as_str(), true)
        }
        ServerMessage::PeerStoppedSharing { peer_id } => {
            on_peer_stopped_sharing(&conn, signals, peer_id.to_string())
        }
        ServerMessage::WatchersChanged {
            ...
            watchers_by_sharer.update(|w| {
                w.insert(sharer_id.to_string(), watchers);
            });
        }
        ServerMessage::Pong => on_pong(&conn),
        ServerMessage::PeerLatency { peer_id, ms } => {
            latency_by_peer.update(|l| {
                l.insert(peer_id.to_string(), ms);
            });
        }
