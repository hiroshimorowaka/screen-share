...
/// Copies the room's invite link to the clipboard and, on success, flips
/// `invite_copied` on for [`COPIED_INDICATOR_MS`] to confirm it visually.
/// Best-effort: a rejected clipboard write (insecure context, no document
...
pub(super) fn invite_click_handler(
    ...
    move |_| copy_invite_link(&room_code, invite_copied)
}
