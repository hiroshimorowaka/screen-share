...
| 6 | Desktop reorg: `desktop/src` → `main/ features/ ipc/ platform/{windows,linux}`; kill inline `process.platform` branches behind a `platform` interface; give `shared-types.ts` contents real owners; `docs/architecture/desktop.md` | medium | `pnpm --dir desktop build` (tsc) clean; `pnpm --dir desktop start` launches; screen-share + audio-share manually verified on Linux (and Windows if available) | in progress |
