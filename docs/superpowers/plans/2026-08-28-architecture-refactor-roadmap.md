...
| 3 | Extract `crates/core` (id/nick/color newtypes + any browser-agnostic room rules) and `crates/signaling` (lib crate; `apps/web` ssr bin depends on it); move `tests/*.rs` to depend on `signaling` | medium | full suite green; `main.rs` wires `signaling::ws::ws_handler` from the new crate |
| 4 | Relocate tests: promote tested items to `pub(crate)`/`pub`, move `#[cfg(test)]` modules into each crate's `tests/` dir (or `src/<mod>/tests.rs` where private access is unavoidable), delete inline test modules | low | identical test count before/after (`cargo test -- --list \| wc -l`); full suite green |
| 5 | `RoomSession` extraction: new `apps/web/src/session/` owning `SignalingClient` + `PeerConnectionManager` + `LocalMedia`; `features/room/*` components reduced to `session.start_sharing()` / `session.watch(peer)` / signal rendering; `ui/client/` → `infra/`; `ui/pages/` → `features/` | medium–high | full suite green; **manual browser test**: create room, two tabs, share + watch both directions, stop via button and via browser control, reload; behavior identical to `main` |
| 6 | Desktop reorg: `desktop/src` → `main/ features/ ipc/ platform/{windows,linux}`; kill inline `process.platform` branches behind a `platform` interface; give `shared-types.ts` contents real owners; add `docs/architecture/desktop.md` | medium | `pnpm --dir desktop build` (tsc) clean; `pnpm --dir desktop start` launches; screen-share + audio-share manually verified on Linux (and Windows if available) |
